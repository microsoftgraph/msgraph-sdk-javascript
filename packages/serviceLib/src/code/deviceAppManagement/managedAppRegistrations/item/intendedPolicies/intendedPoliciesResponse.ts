...
export class IntendedPoliciesResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["@odata.nextLink", (o, n) => { (o as unknown as IntendedPoliciesResponse).nextLink = n.getStringValue(); }],
            ["value", (o, n) => { (o as unknown as IntendedPoliciesResponse).value = n.getCollectionOfObjectValues<ManagedAppPolicy>(ManagedAppPolicy); }],
        ]);
    };
    /**
