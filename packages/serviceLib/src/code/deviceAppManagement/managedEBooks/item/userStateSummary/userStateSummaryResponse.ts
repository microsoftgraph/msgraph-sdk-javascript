...
export class UserStateSummaryResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["@odata.nextLink", (o, n) => { (o as unknown as UserStateSummaryResponse).nextLink = n.getStringValue(); }],
            ["value", (o, n) => { (o as unknown as UserStateSummaryResponse).value = n.getCollectionOfObjectValues<UserInstallStateSummary>(UserInstallStateSummary); }],
        ]);
    };
    /**
