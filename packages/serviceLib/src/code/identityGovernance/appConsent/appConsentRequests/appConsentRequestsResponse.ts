...
export class AppConsentRequestsResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["@odata.nextLink", (o, n) => { (o as unknown as AppConsentRequestsResponse).nextLink = n.getStringValue(); }],
            ["value", (o, n) => { (o as unknown as AppConsentRequestsResponse).value = n.getCollectionOfObjectValues<AppConsentRequest>(AppConsentRequest); }],
        ]);
    };
    /**
