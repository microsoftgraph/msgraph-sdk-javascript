...
export class SubscribedSkusResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["@odata.nextLink", (o, n) => { (o as unknown as SubscribedSkusResponse).nextLink = n.getStringValue(); }],
            ["value", (o, n) => { (o as unknown as SubscribedSkusResponse).value = n.getCollectionOfObjectValues<SubscribedSku>(SubscribedSku); }],
        ]);
    };
    /**
