...
export class WorksheetsResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["@odata.nextLink", (o, n) => { (o as unknown as WorksheetsResponse).nextLink = n.getStringValue(); }],
            ["value", (o, n) => { (o as unknown as WorksheetsResponse).value = n.getCollectionOfObjectValues<WorkbookWorksheet>(WorkbookWorksheet); }],
        ]);
    };
    /**
