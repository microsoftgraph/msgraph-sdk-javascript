...
export class SecureScoresResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["@odata.nextLink", (o, n) => { (o as unknown as SecureScoresResponse).nextLink = n.getStringValue(); }],
            ["value", (o, n) => { (o as unknown as SecureScoresResponse).value = n.getCollectionOfObjectValues<SecureScore>(SecureScore); }],
        ]);
    };
    /**
