...
export class DetectedAppsResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["@odata.nextLink", (o, n) => { (o as unknown as DetectedAppsResponse).nextLink = n.getStringValue(); }],
            ["value", (o, n) => { (o as unknown as DetectedAppsResponse).value = n.getCollectionOfObjectValues<DetectedApp>(DetectedApp); }],
        ]);
    };
    /**
