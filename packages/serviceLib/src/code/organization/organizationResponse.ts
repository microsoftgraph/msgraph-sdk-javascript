...
export class OrganizationResponse implements Parsable {
    ...
    public getFieldDeserializers<T>() : Map<string, (item: T, node: ParseNode) => void> {
        return new Map<string, (item: T, node: ParseNode) => void>([
            ["@odata.nextLink", (o, n) => { (o as unknown as OrganizationResponse).nextLink = n.getStringValue(); }],
            ["value", (o, n) => { (o as unknown as OrganizationResponse).value = n.getCollectionOfObjectValues<Organization>(Organization); }],
        ]);
    };
    /**
