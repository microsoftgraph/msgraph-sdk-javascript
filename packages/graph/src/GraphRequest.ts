 ...
 * @module GraphRequest
 */
import { AuthenticationProvider, HttpMethod, RequestInformation, RequestOption } from "@microsoft/kiota-abstractions";
...
export class GraphRequest {
    ...
    private fetchRequestAdapter: FetchRequestAdapter;
    ...
    public constructor(httpClient: HttpClient, authProvider: AuthenticationProvider, config: ClientOptions, path: string) {
        ...
        this.parsePath(path);
        this.fetchRequestAdapter = new FetchRequestAdapter(this.authenticationProvider, null, null, this.httpClient);
    }
    ...
    public async getNew(queryOptions: QueryOptions, headers: Record<string, string>, requestOptions: Record<string, RequestOption>, callback?: GraphRequestCallback): Promise<any> {
        if (queryOptions) {
            for (const key in queryOptions) {
                this.urlComponents.oDataQueryParams[key] = queryOptions[key];
            }
        }

        const requestInfo = new RequestInformation();
        requestInfo.URL = this.buildFullUrl();
        ...
        return this.fetchRequestAdapter.send(requestInfo, callback);
    }
